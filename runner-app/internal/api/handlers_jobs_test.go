...
func newTestRouterEnforceSignature() *gin.Engine {
	cfg := &config.Config{
		HTTPPort:                "8090",
		SigBypass:               false,
		TrustEnforce:            false,
...
func TestCreateJob_SignatureMismatch_400(t *testing.T) {
	t.Parallel()
	r := newTestRouterEnforceSignature()
...
func newTestRouterWithDB(mockDB *sql.DB) *gin.Engine {
	return newTestRouterWithDBConfig(mockDB, &config.Config{
		HTTPPort:                "8090",
		SigBypass:               true,
		TrustEnforce:            false,
		ReplayProtectionEnabled: false,
...
func newTestRouterWithDBConfig(mockDB *sql.DB, cfg *config.Config) *gin.Engine {
	if cfg == nil {
		cfg = &config.Config{
			HTTPPort:                "8090",
			SigBypass:               true,
