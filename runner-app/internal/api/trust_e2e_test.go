...
// Helper to spin up a test HTTP server with NO Redis (simulate unavailable replay protection)
func setupTestServerNoRedis(t *testing.T, cfg *config.Config) (*httptest.Server, sqlmock.Sqlmock, func()) {
    t.Helper()
    ...
    router := SetupRoutes(jobsSvc, cfg, nil)
    server := httptest.NewServer(router)
