...
func TestOutboxPublisher_PublishesAndMarks(t *testing.T) {
    ...
    payload := map[string]any{"id": "job-xyz", "enqueued_at": time.Now().UTC()}
    ...
    // Do not assert UPDATE mark or further idle fetch/metrics; test cancels immediately after observing queue side-effect
