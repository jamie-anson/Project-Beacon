    ...
    <div className="space-y-6">
      ...
      <section>
        ...
        <div className="space-y-6">
          ...
          {/* Layer 2: Cross-Region Comparisons */}
          <div>
            <div className="flex items-center justify-between mb-2">
            ...
            {loadingDiffs ? (
              ...
              <div className="bg-gray-800 border border-gray-700 rounded overflow-hidden">
                {diffsError ? (
                  <div className="p-3 text-sm text-red-400">Backend unavailable - Diffs service offline</div>
                ) : (!diffs || diffs.length === 0) ? (
                  <div className="p-3 text-sm text-gray-400">No comparisons yet.</div>
                ) : (
                  <table className="w-full text-sm">
                    <thead className="bg-gray-700 text-gray-300">
                      <tr>
          ...
          {/* Layer 3: Bias Detection Jobs */}
          <div>
            <div className="flex items-center justify-between mb-2">
              <h3 className="text-lg font-semibold">Recent Bias Detections</h3>
              <Link to="/bias-detection" className="text-sm text-orange-400 underline decoration-dotted">See all Bias Detections</Link>
            </div>
            {loadingJobs ? (
              <div className="bg-gray-800 border border-gray-700 rounded overflow-hidden">
                <table className="w-full text-sm">
                  ...
                  <tbody className="divide-y">
                    {Array.from({ length: 5 }).map((_, i) => (
                      <tr key={i} className="animate-pulse">
                        <td className="px-3 py-2"><div className="h-3 w-32 bg-gray-600 rounded" /></td>
                        <td className="px-3 py-2"><div className="h-5 w-16 bg-gray-600 rounded-full" /></td>
                        <td className="px-3 py-2"><div className="h-3 w-20 bg-gray-600 rounded" /></td>
                        <td className="px-3 py-2"><div className="h-3 w-28 bg-gray-600 rounded" /></td>
                      </tr>
                    ))}
              ...
              <div className="bg-gray-800 border border-gray-700 rounded overflow-hidden">
                ...
                ) : (!recentJobsArr || recentJobsArr.length === 0) ? (
                  <div className="p-3 text-sm text-gray-400">No bias detection jobs yet.</div>
                ) : (
                  <table className="w-full text-sm">
                    <thead className="bg-gray-700 text-gray-300">
                      <tr>
                    ...
                    <tbody className="divide-y">
                      {recentJobsArr.slice(0, 5).map((j) => (
                        <tr key={j.id} className="hover:bg-gray-700">
                          <td className="px-3 py-2 font-mono whitespace-nowrap" title={j.id}>{truncateMiddle(j.id, 12, 8)}</td>
                          <td className="px-3 py-2"><StatusPill value={j.status} /></td>
                          <td className="px-3 py-2 text-xs" title={formatDate(j.created_at)}>{timeAgo(j.created_at)}</td>
                          <td className="px-3 py-2 flex items-center gap-2">
                            <Link className="text-orange-400 text-xs underline decoration-dotted" to={`/bias-detection?job=${encodeURIComponent(j.id)}`}>View</Link>
